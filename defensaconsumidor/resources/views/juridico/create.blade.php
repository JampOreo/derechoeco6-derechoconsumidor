...
<html lang="es">
<head>
    <title>Agregar Contenido Jurídico</title>
    <style>
...
<body>
    <h1>Agregar Contenido Jurídico</h1>
    <form method="POST" action="{{ route('juridico.store') }}">
        @csrf
        <input type="text" name="titulo" placeholder="Título (opcional)">
        <textarea name="contenido" rows="10" placeholder="Contenido jurídico" required></textarea>
